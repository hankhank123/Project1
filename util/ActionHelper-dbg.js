...
sap.ui.define([
	...
	return Object.extend("cross.fnd.fiori.inbox.util.ActionHelper", {
		...
		isIntentURL: function(sURL) {
			...
			return this._oURLParsingService && this._oURLParsingService.isIntentUrl(sURL) ? true : false;
		},
		
		isAnnotationBasedTask: function(sURL) {
			var oParameters = Parser.fnParseComponentParameters(sURL);
			return (!jQuery.isEmptyObject(oParameters)) ? true : false;
		},
		...
		_getThemeandLanguageLocaleParams: function(){
			...
			var sThemeNWBC =  sap.ushell.Container ? sap.ushell.Container.getUser().getTheme(sap.ushell.User.prototype.constants.themeFormat.NWBC) : null;
		...
		getEncodedURL: function(sURL) {
			...
			sURL = aURLParts[0] + "?" + jQuery.sap.encodeURLParameters(this.getQueryObject(aURLParts[1]));
			return sURL;
		},
		...
		getQueryObject: function( sQueryString ) {
		    ...
		    jQuery.each(aQueryParts, jQuery.proxy(function(i, sQueryPart) {
		...
		// set the reference (this) of the controller while calling this function. 
		// make sure that promise to load metadata is resolved before calling this function.
		// TODO make a generic function to check the properties in metadata
		fnCheckGUILinkPropertySupported: function() {
			if (this._bGUILinkPropertyInMetadata == null) {
				this._bGUILinkPropertyInMetadata = false;
				var oTaskEntity = this.oDataManager.oServiceMetaModel.getODataEntityType("TASKPROCESSING.Task");
				if(oTaskEntity){
					if(this.oDataManager.oServiceMetaModel.getODataProperty(oTaskEntity, this.GUILinkProperty)){
