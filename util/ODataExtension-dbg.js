...
], function(Object, ODataModel, ODataListBindingExtension) {
	"use strict";
	return Object.extend("cross.fnd.fiori.inbox.ODataExtension", {
		...
		overrideProcessSuccess : function(oDataModel) {
			if(oDataModel){
				oDataModel._processSuccess = function(oRequest, oResponse, fnSuccess, mGetEntities, mChangeEntities, mEntityTypes) {
					...
					if (!jQuery.sap.startsWith(sUrlPath,'/')) {
						sUrlPath = '/' + sUrlPath;
					}
					...
					if(!isNaN(ui5Version) && ui5Version <= 1.40 && sUrl.indexOf("TaskCollection") > -1 && sUrl.indexOf("$expand") > -1) {
						try {
							...
							var masterController = sap.ca.scfld.md.app.Application.getImpl().oCurController.MasterCtrl; //The S2 controller
