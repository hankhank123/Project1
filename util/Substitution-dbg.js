...
cross.fnd.fiori.inbox.Substitution = (function() {
	
	var _oDataManager = null;
	var _aUserPictureAvailability = {};
	var userDetailsInfo = null;
	
	return {
		
		formatterUserIconVisibility: function(sUserId, sSAP__Origin) {
			this.getParent().data('userid',sUserId);
			var sUrl = this.getModel().sServiceUrl +"/UserInfoCollection(SAP__Origin='" + sSAP__Origin + "',UniqueName='" + sUserId + "')/$value";
			return !(cross.fnd.fiori.inbox.Substitution._isImagePresent(sUrl));
		},
		
		formatterUserPictureVisibility: function(sUserId, sSAP__Origin) {
			this.getParent().data('userid',sUserId);
			var sUrl = this.getModel().sServiceUrl +"/UserInfoCollection(SAP__Origin='" + sSAP__Origin + "',UniqueName='" + sUserId + "')/$value";
			return cross.fnd.fiori.inbox.Substitution._isImagePresent(sUrl);
		},
		...
		_processSubstitutedUsersCollection: function(oSubstitutedUser) {
		 	var oNewEntry = {
						...
						SAP__Origin: oSubstitutedUser.SAP__Origin
			};
			return oNewEntry;
		},
		...
		_getSubstitutedUserFilters : function(aSubstitutedUserFilterKeys){
			...
			for (var i = 0; i < aSubstitutedUserFilterKeys.length; i++) {
				...
				if(aSubstitutedUserFilterKeys[i] === ""){
					var nullFilter = new sap.ui.model.Filter({
						path: "SubstitutedUser",
						operator: sap.ui.model.FilterOperator.EQ,
						value1: null
					});
		...
		formatterUserPictureUrl: function(sUserId, sSAP__Origin) {
			return this.getModel().sServiceUrl +"/UserInfoCollection(SAP__Origin='" + sSAP__Origin + "',UniqueName='" + sUserId + "')/$value";
		},
		...
		formatterSubstitutionStartDate : function(sDate) {
			return sap.ui.core.format.DateFormat.getDateInstance({style:"short"}).format(sDate);
		},
		
		formatterUserPictureIcon: function(sMediaSrc) {
			return cross.fnd.fiori.inbox.Conversions.formatterUserIconForSubstitutors(sMediaSrc);
		},
		...
		formatterSubstitutionEndDate: function(sBeginDate,sEndDate) {
				...
				return (iTotalDays>3650) ? "" : sap.ui.core.format.DateFormat.getDateInstance({style:"short"}).format(sEndDate);
		...
		_isCurrentDate: function(dateValue) {
			if (dateValue) {
				...
				if ((oCurrentDate.getDate() == dateValue.getDate()) && (oCurrentDate.getMonth() == dateValue.getMonth()) && (oCurrentDate.getYear() == dateValue.getYear())) {
					return true;
				}
			}			
		},
		...
		_isFutureDate: function(dateValue) {
			if (dateValue) {
				if (cross.fnd.fiori.inbox.Substitution._getDiffWithCurrentTime(dateValue) > 0 && !cross.fnd.fiori.inbox.Substitution._isCurrentDate(dateValue)) {
					return true;
				}
			}
			return false;
		...
		formatterSubstitutesStatus: function (sUser, bActive, sMode, sBeginDate) {
			...
			if (sUser) {
				...
				if((sMode !== "TAKE_OVER")&&(userDetailsInfo[sUser] != undefined)) {
					if (sap.ui.Device.system.phone) {
						...
						return bInCurrentDateRange ? sActiveStatusText
							: sInactiveStatusText;
					} else {
							return userDetailsInfo[sUser].bActive ? sActiveStatusText
							: sInactiveStatusText;
					}
	    ...
	    formatterSubstitutedUserName: function(sId, sName) {
		 ...
		 formatterSubstitutedUserPicture : function(sUserId, sSAP__Origin) {
			 ...
			 var url = _oDataManager.oModel.sServiceUrl +"/UserInfoCollection(SAP__Origin='" + sSAP__Origin + "',UniqueName='" + sUserId + "')/$value";
			 if (_aUserPictureAvailability[url] == undefined) {
			 ...
			 if(bImagePresent) {
				 ...
				 that.destroy();
				 url = sap.ui.core.IconPool.getIconURI("person-placeholder");
